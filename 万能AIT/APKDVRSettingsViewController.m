...
@property (weak, nonatomic) IBOutlet UILabel *OTAUpgradeL;
...
- (void)viewDidLoad {
    ...
    self.OTAUpgradeL.text = NSLocalizedString(@"OTA升级", nil);
...
//    [self.clipDurationSegment setTitle:NSLocalizedString(@"5分钟", nil) forSegmentAtIndex:2];
    ...
    [self.parkingModeSeg setTitle:NSLocalizedString(@"关", nil) forSegmentAtIndex:0];
    [self.parkingModeSeg setTitle:NSLocalizedString(@"开", nil) forSegmentAtIndex:1];
...
//        }];
        ...
        HUD.label.text = NSLocalizedString(@"升级文件准备中，请稍后...", nil);
        self.HUD = HUD;
...
-(void)excuteOTAUpgrate
{
    ...
    __weak typeof (self) weakSelf = self;
    NSString *urlStr = [NSString stringWithFormat:@"http://ota.apical-hk.com/download/?pr=18820&cus=DIrected_NCPBDVR&version=%@&visitor=%@",versonStr,str];
...
- (IBAction)toggleTimelapseSwitch:(UISwitch *)sender {
    ...
    [[APKDVRCommandFactory setCommandWithProperty:property value:value] execute:^(id responseObject) {
        ...
        [hud hideAnimated:YES];
        if (rval == 701) {
            [APKAlertTool showAlertInViewController:weakSelf title:nil message:@"没有ACC，无法设置此功能" confirmHandler:^(UIAlertAction *action) {
                sender.on = !sender.on;
            }];
        }
        [APKAlertTool showAlertInViewController:weakSelf title:nil message:NSLocalizedString(@"设置失败！", nil) confirmHandler:^(UIAlertAction *action) {
...
- (NSArray *)rowHeights{
    ...
    if (!_rowHeights) {
        _rowHeights = @[@(62),@(62),@(98),@(98),@(98),@(98),@(98),@(62),@(62),@(62),@(62),@(62),@(62),@(62),@(62)];
    }
    return _rowHeights;
}
...
//        _cells = @[self.recordSoundCell,self.motionDetectionCell,self.clipDurationCell,self.gSensorCell,self.LCDLightCell,self.watermarkCell1,self.dateFormatCell,self.exposureCell,self.correctCameraClockCell,self.formatCell,self.networkConfigureCell,self.factoryResetCell,self.helpCell,self.aboatCell,self.DBValueCell];
        _cells = @[self.recordSoundCell,self.TimelapseCell,self.EdogCell,self.parkingModeCell,self.clipDurationCell,self.gSensorCell,self.volumeCell,self.factoryResetCell,self.correctCameraClockCell,self.networkConfigureCell,self.formatCell,self.aboatCell,self.helpCell,self.OTAUpgradeCell];
    }
    return _cells;
}
